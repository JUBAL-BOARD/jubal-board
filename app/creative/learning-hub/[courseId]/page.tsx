...
const CourseDetailPage = () => {
  ...
  // Default to first module open + first video resource active once data loads
  useEffect(() => {
  ...
  if (error || courseError) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-red-500">Error loading: {error ?? courseError}</p>
      </div>
    );
  }

  // Fall back to store data (e.g. while courseId-based fetch isn't wired up yet)
  ...
  const allResources = (activeCourse.modules ?? []).flatMap((m) => m.resources);
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 w-full px-4 lg:px-7 py-6 overflow-y-auto">
            ...
            {!activeResource ? (
              ...
              <>
                <video
                  ...
                  poster={activeResource.thumbnailUrl ?? activeCourse.thumbnailUrl}
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-2">{activeCourse.title}</h2>
              <div className="flex items-center gap-3 text-sm">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-gray-700">{activeCourse.rating}</span>
                </div>
                <span className="px-2 py-0.5 bg-[#1c1c3a] text-white text-xs font-semibold rounded">{activeCourse.level}</span>
                <span className="text-gray-500">Duration: {activeCourse.duration}</span>
              </div>
            </div>
            <span className="px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-lg">
              {activeCourse.isFree || activeCourse.cost === 0 ? "Free Course" : `$${activeCourse.cost}`}
            </span>
          </div>
          ...
          {/* Course Outline — now built from modules/resources */}
          <div className="mb-5">
          ...
          {showCertificate && (
            <div className="bg-[#fafafa] border border-gray-100 rounded-xl p-6 mb-10 relative">
              ...
              <div className="border-2 border-teal-200 rounded-lg p-8 max-w-lg mx-auto mb-6 bg-white relative">
                ...
                <p className="text-center text-sm font-bold text-gray-800 mb-6">{activeCourse.title}</p>
