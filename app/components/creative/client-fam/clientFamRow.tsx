...
const ClientFamRow: React.FC<Props> = ({ client, isLast, onChat }) => {
  return (
    <div className={`flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-4 px-4 lg:px-5 py-4 bg-[#fafafa] hover:bg-white transition-colors ${!isLast ? "border-b border-gray-100" : ""}`}>
      ...
      <div className="flex flex-row lg:flex-col gap-2 flex-shrink-0">
        <Link href={`/creative/client-fam/${client.id}`}>
        ...
        <button
          onClick={onChat}
          className="flex-1 lg:flex-none flex items-center justify-center gap-1.5 bg-[#1a1a2e] hover:bg-[#2a2a4e] text-white text-xs font-semibold font-body px-4 py-1.5 rounded-lg transition-colors"
        >
