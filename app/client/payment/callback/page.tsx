...
// Same UUID-extraction pattern already used in the wallet page's
// extractProjectId — tx_ref is shaped "proj_<projectId>_<timestamp>"
...
// Confirmed against the real ProjectDetail status enum used in
// app/client/my-desk/[projectId]/page.tsx:
// IN_PROGRESS | COMPLETED | REVISION | PENDING_PAYMENT | PARTIALLY_COMPLETED
const PENDING_PAYMENT_STATUS = "PENDING_PAYMENT";

const POLL_INTERVAL_MS = 2500;
const MAX_POLL_ATTEMPTS = 8; // ~20s total

const PaymentCallbackPage: React.FC = () => {
  const router = useRouter();
  const searchParams = useSearchParams();

  const status = searchParams.get("status"); // "successful" | "failed" | "cancelled" (Flutterwave-reported)
  ...
  useEffect(() => {
    // Flutterwave itself reported a non-success outcome — no point polling.
    ...
    const poll = async () => {
      try {
        ...
        if (!res.ok) {
          // transient fetch failure — just retry, don't fail outright
          scheduleNext();
          return;
        }
        ...
        // Webhook has credited escrow and flipped status away from pending-payment
